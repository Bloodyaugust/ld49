...
func _ready():
  ...
  var _new_resource_types_state = Store.state.resource_types.duplicate(true)
  _new_resource_types_state[type] += 1
  Store.set_state("resource_types", _new_resource_types_state)
  ...
  if type == "water":
    ...
    var _num_resources_requirement_type = Store.state.resource_types[game_resource_data[resource_name].spawner_requirements.type]
    var _num_wanted_spawners = _num_resources_requirement_type / game_resource_data[resource_name].spawner_requirements.amount

    print("num wanted spawners: " + str(_num_wanted_spawners))
    print("num spawners: " + str(Store.state.spawners[game_resource_data[resource_name].spawner_requirements.spawner_type]))
    if _num_wanted_spawners > Store.state.spawners[game_resource_data[resource_name].spawner_requirements.spawner_type]:
      var _new_spawner:Node2D = game_resource_data[resource_name].spawner_scene.instance()
      var _resources:Array = get_tree().get_nodes_in_group("resources")
      var _resource_positions:Array = []

      for _resource in _resources:
        if _resource.type == game_resource_data[resource_name].spawner_requirements.type:
